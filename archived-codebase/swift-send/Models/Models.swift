...
extension Conversation {
    ...
    /// Content-based identifier for SwiftUI list updates
    /// Changes when name or members change (NOT messages - those update via array diffing)
    /// This ensures NavigationLinks remain stable while in a conversation
    var contentIdentifier: String {
        let nameHash = name?.hashValue ?? 0
        let membersHash = memberIds.joined(separator: ",").hashValue
        return "\(safeId)-\(nameHash)-\(membersHash)"
    }
...
struct Message: Identifiable, Codable, Equatable {
    ...
    func isDeletedForUser(_ userId: String) -> Bool {
        let isDeleted = deletedFor?.contains(userId) ?? false
        if isDeleted {
            print("🚫 [Message] Message \(id) is deleted for user \(userId)")
        }
        return isDeleted
    }
